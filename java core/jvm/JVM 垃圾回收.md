...
JVM的垃圾回收机制是Java中比较重要的知识点，也是面试官常考的问题，本文主要围绕以下面试题来讲解JVM的垃圾回收机制。
...
![1539251145709](../../assets/1539251145709.png)

当需要排查各种 内存溢出问题、当垃圾收集成为系统达到更高并发的瓶颈时，我们就需要对这些“自动化”的技术实施必要的监控和调节。另外JVM作为面试必备，掌握JVM垃圾回收对我们来说也就理所当然了！
...
![1539251192394](../../assets/1539251192394.png)
...
![1539251293394](../../assets/1539251293394.png)

### 1.1 对象优先在Eden区分配

目前主流的垃圾收集器都会采用分代回收算法，其他有标记-清除、复制、标记-整理，因此需要将堆内存分为新生代和老年代，这样我们就可以根据各个年代的特点选择合适的垃圾收集算法。
...
![1539251773836](../../assets/1539251773836.png)

添加的参数： `-XX:+PrintGCDetails`![img](../../assets/640)

运行结果：

![1539251819245](../../assets/1539251819245.png)
...
![1539251894505](../../assets/1539251894505.png)
...
public class GCTest {    
    public static void main(String[] args) {        
        byte[] allocation1, allocation2,allocation3,allocation4,allocation5;        		allocation1 = new byte[32000*1024];        
...
![1539252457432](../../assets/1539252457432.png)
...
![1539252540047](../../assets/1539252540047.png)
...
![1539253053046](../../assets/1539253053046.png)
...
![1539253120629](../../assets/1539253120629.png)

### 3.2 复制算法

为了解决效率问题，“复制”收集算法出现了。它可以将内存分为大小相同的两块，每次使用其中的一块。当这一块的内存使用完后，就将还存活的对象复制到另一块去，然后再把使用的空间一次清理掉。这样就使每次的内存回收都是对内存区间的一半进行回收。

![1539253179677](../../assets/1539253179677.png)

### 3.3 标记-整理算法

根据老年代的特点特出的一种标记算法，标记过程仍然与“标记-清除”算法一样，但后续步骤不是直接对可回收对象回收，而是让所有存活的对象向一段移动，然后直接清理掉端边界以外的内存。

![1539253272942](../../assets/1539253272942.png)
...
![1539253447944](../../assets/1539253447944.png)
...
![1539253647008](../../assets/1539253647008.png)
...
![1539253767582](../../assets/1539253767582.png)
...
**新生代采用复制算法，老年代采用标记-整理算法。**![img](https://mmbiz.qpic.cn/mmbiz_jpg/hvUCbRic69sA2CjiboOtVAHLxicMm9n3AVqIT5bicWpicR0ic4x9x4AgY7p842Rxyqicj0VJMJVvrEMdkib3Yz8LMrVuhA/640?tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![1539254275235](../../assets/1539254275235.png)
